...
def main():
    ...
    atk = PGDAttack(model, data, device, clamp=CLAMP)
    x_adv = atk.attack(targets, eps=EPS, alpha=ALPHA, steps=STEPS,
                       random_start=RANDOM_START, targeted=TARGETED, target_label=TARGET_LABEL)
    ...
    print(f"PGD | eps={EPS} alpha={ALPHA} steps={STEPS} | TARGETED={TARGETED} | TARGET_LABEL={TARGET_LABEL}")
