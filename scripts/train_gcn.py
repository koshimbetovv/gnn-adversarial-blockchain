...
CONFIG = {
    ...
    "training": {
        ...
        "weight_decay": 5e-4,
        "grad_clip": 1.0,
        "adam_eps": 1e-5,     # helps stability (esp. MPS)
        "log_every": 50,      # print train/val P/R/F1 every N epochs
    },
    "save": {
        "save_dir": "models",
        "save_run": True,      # if True -> models/gcn_YYYYMMDD_HHMMSS/
        "filename": "gcn.pt"   # used when save_run=False
    }
}
...
def safe_class_weights(y_train, device):
    """
    Class weights computed on TRAIN only. If a class is missing in train, disable weights.
    """
    num_classes = 2
...
def eval_split(model, data, split: str):
    ...
    # Per-class metrics (labels: 0 legit, 1 fraud)
...
def main():
    ...
    print("\n=== Training (printing train/val P/R/F1 during training) ===")
