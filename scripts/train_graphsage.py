...
def parse_args():
    parser = argparse.ArgumentParser(description="Train GraphSAGE with hyperparameters")
    
    # Model params
    parser.add_argument("--hidden_dim", type=int, default=64)
    # Note: num_layers seems not configurable in original GraphSAGE call logic? 
    # Checking original file: model = GraphSAGE(..., hid_dim=..., aggr=...). 
    # Let's check src/models/graphsage.py signature if I need to support num_layers.
    # User prompt said "num_layers: categorical [2, 3, 4]".
    # I should verify GraphSAGE model supports num_layers. If not, I can't tune it without changing arch code (forbidden).
    # I'll check it shortly. For now assuming it might only be 2 layer fixed if not exposed.
    # Wait, I better check before writing this file. But let's assume I need to pass it if possible.
    # I'll check the file content stored in my memory... 
    # Original train_graphsage.py did NOT pass num_layers.
    # Ref: `src/models/graphsage.py` needs to be checked.
    parser.add_argument("--aggr", type=str, default="mean", choices=["mean", "max", "sum"])
    parser.add_argument("--num_layers", type=int, default=2) # speculative, will check support
    parser.add_argument("--dropout", type=float, default=0.0) # speculative, original config didn't have it? Wait, let's check.
    ...
    parser.add_argument("--trial_id", type=str, default="default")
    parser.add_argument("--git_hash", type=str, default="unknown")
...
def main():
    ...
    # Initialize model
    # I need to verify if GraphSAGE accepts num_layers and dropout. 
    # This write attempt assumes it might, but if I'm wrong I'll fix it. 
    # Actually, let's verify `src/models/graphsage.py` first to be safe.
    # The original script call was:
    # model = GraphSAGE(
    ...
    # )
    # So I suspect it takes fixed layers.
    # If so, I cannot tune num_layers for GraphSAGE unless I change the model definition (forbidden).
    # I will stick to the original call signature for now and check afterwards if I can enhance it.
    ...
    if y_train.numel() == 0:
        raise ValueError("Train split has 0 labeled nodes.")
    ...
    for epoch in range(1, CONFIG["training"]["epochs"] + 1):
        ...
        if not torch.isfinite(loss):
            print(f"Epoch {epoch}: Loss is not finite. Stopping.")
            break
    ...
    print("\n=== GraphSAGE Final Evaluation (Best Model) ===")
    ...
    out_path = os.path.join(save_dir, f"graphsage_trial_{args.trial_id}.pt")
    ...
    if CONFIG["save"]["save_run"] and not args.results_file:
        ...
        out_dir = os.path.join(save_dir, f"graphsage_{run_id}")
