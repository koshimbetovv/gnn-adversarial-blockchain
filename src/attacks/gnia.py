...
def gumbel_topk(logits, budget, tau, random_flag, eps, device):
    mask_val = 1e9
    cur_mask = torch.zeros_like(logits)
    final_discrete = torch.zeros_like(logits).to(device)
    tmp = None
    
    for i in range(budget):
        if i > 0:
            val, idx = torch.max(tmp, dim=0)
            cur_mask[idx] = mask_val
            
        log_p = logits - cur_mask
        tmp = gumbel_softmax(log_p, tau, random_flag, eps)
        final_discrete = final_discrete + tmp
        
    return final_discrete
...
class AttrGeneration(nn.Module):
    def __init__(self, labels, tau, feat_dim, weight1, weight2, discrete, device, tar_num, feat_max, feat_min):
        ...
        self.labels = labels
        self.label_dim = int(labels.max().item() + 1)
    ...
    def pool_func(self, wlabel, wsec):
        ...
        if self.tar_num == 1:
            if wlabel.dim() == 1: wlabel = wlabel.unsqueeze(0)
            if wsec.dim() == 1: wsec = wsec.unsqueeze(0)
    ...
    def forward(self, target, feat, sub_graph_nodes, node_emb, wlabel, wsec, feat_num=None, eps=1, train_flag=False):
        ...
        if self.discrete == True:
            ...
            if self.feat_max is not None and self.feat_min is not None:
...
class EdgeGeneration(nn.Module):
    def __init__(self, labels, feat_dim, weight1, weight2, device, tar_num=1, tau=None):
        ...
        self.labels = labels
        self.label_dim = int(self.labels.max() + 1)
        self.feat_dim = feat_dim
        self.weight1 = weight1
        self.weight2 = weight2
        self.tar_num = tar_num
        ...
        if self.tar_num == 1:
            ...
            if self.adj_tensor.is_sparse:
                ...
                for t in self.target:
                    # Sparse indexing [t] returns a sparse vector (row t)
                    # Convert only this row to dense (size N, not N^2)
                    row_t = self.adj_tensor[t.item()].to_dense()
                    val = row_t[self.sub_graph_nodes].unsqueeze(1)
                    norm_a_targets_list.append(val)
...
class GNIA(nn.Module):
    def __init__(self, labels, feat_dim, weight1, weight2, discrete, device, tar_num=1, feat_max=None, feat_min=None, feat_num=None, attr_tau=1.0, edge_tau=1.0):
    ...
    def forward(self, target, sub_graph_nodes, budget, feat, nor_adj_tensor, node_emb, wlabel, wsec, train_flag, eps=0):
        ...
        self.new_feat, self.add_feat = self.add_node_agent(self.target, self.feat, self.sub_graph_nodes, self.node_emb, wlabel, wsec, self.feat_num, eps, train_flag=train_flag)

        # 2. Edge Generation
        self.score, self.masked_score_idx = self.add_edge_agent(self.budget, self.target, self.sub_graph_nodes, self.new_feat, self.nor_adj_tensor, wlabel, wsec, eps, train_flag=train_flag)

        # Evaluation / Inference Discrete
        self.disc_feat = None
        self.disc_score = None
        
        if train_flag:
            self.disc_score = self.score
            # For attributes, we keep soft in training
        else:
            if self.discrete:
        ...
        return self.add_feat, self.disc_score, self.masked_score_idx, self.new_feat

class GNIAAttack(BaseAttack):
    def __init__(self, model, data, device, args=None):
        super().__init__(model, data, device)
        self.args = args if args else {}
        self.gnia_model = None
        
    @staticmethod
    def extract_weights(model):
        """
        Extract W1 and W2 from standard GCN/GAT/SAGE models.
        Returns: W1, W2 (torch.Tensor)
        """
        weights = []
        ...
        # Fallback if specific naming fails, just take first and last large weights
        if len(weights) >= 2:
            w1 = weights[0]
            w2 = weights[-1]
            return w1, w2.t() 
    ...
    def attack(self, target_idx, **kwargs):
        # Placeholder for structured attack call
        pass
